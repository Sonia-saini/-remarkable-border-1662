...
export default function Login(){
    const {states,hanglelog,log,logs}=useContext(loginContext)
    const {email,name,password}=logs;
    if(states.isAuth){
        return <Navigate to="/"/>
     }
    return (
  ...
  <AlertDescription> If you fill Anything wrong detail , it will create problem.</AlertDescription>
        ...
        <div style={{border:"1px solid",width:"400px",height:"400px",gap:"30px", padding: "50px",marginTop:"-590px"}}>
            <h2 id="h2">LOG IN</h2>
            <input type="text" placeholder="E-MAIL" id="email" className="input" value={email} name="email" onChange={hanglelog}/><br/>
            <input type="text" placeholder="USERNAME" id="email" className="input" value={name} name="name" onChange={hanglelog}/><br/>

            <input type="password" placeholder="PASSWORD" id="password" value={password} className="input"name="password" onChange={hanglelog}/>
            <p id="p">
